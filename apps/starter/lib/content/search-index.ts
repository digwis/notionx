import { matchesSearchQuery, normalizeSearchQuery } from "./search.ts";
import type { ContentModelId } from "./models.ts";
import type { SqlDatabaseAdapter } from "../platform/runtime.ts";
...
export async function deleteSearchIndexDocument(
  db: SqlDatabaseAdapter,
  input: { modelId: ContentModelId | string; routeId: string }
) {
  await db
    .prepare(
...
export async function deleteSearchIndexForModel(
  db: SqlDatabaseAdapter,
  input: { modelId: ContentModelId | string }
) {
  await db
...
export async function getMissingSearchIndexRouteIds(
  db: SqlDatabaseAdapter,
  input: { modelId: ContentModelId | string; routeIds: readonly string[] }
) {
...
export async function querySearchIndexRouteIds(
  db: SqlDatabaseAdapter,
  input: { modelId: ContentModelId | string; query: string; limit?: number }
) {
