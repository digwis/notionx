...
> 范围：消费 `@vinext/foundation` 的项目如何获得修复与新能力。三类升级：
> patch/minor 自动；major 手动；canary 自行取用。

## 1. Dependabot 配置（推荐）

每个消费项目仓库根放一个 `.github/dependabot.yml`，让 GitHub 监听
`@vinext/foundation` 的新版本。下面是一份最小可用配置（仓库里目前还没有这
个文件，需要时直接添加）：
...
updates:
  - package-ecosystem: "npm"
    ...
    schedule:
      interval: "weekly"
      day: "monday"
    # 只关心 @vinext/foundation；其它依赖可以并行开启其它 update 条目
    allow:
      - dependency-name: "@vinext/foundation"
...
阅读 [Foundation Changelog](./foundation-changelog.md) 中的迁移说明。
...
```bash
pnpm update @vinext/foundation
# 或锁定到具体版本
pnpm add @vinext/foundation@^1.2.0
```
...
合入升级后，请把 [Foundation Changelog](./foundation-changelog.md) 里那段
