...
脚手架生成的项目已经注册了一个 `blog` 内容源。下面以"再加一个
...
打开 `lib/content/models.ts`，在已有 `blogContentModel` 旁边追加
...
`route.ts` 的形状与 `app/api/blog/route.ts` 完全一致，只是把
