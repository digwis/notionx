...
> `@vinext/foundation` 的发布说明。完整的 GitHub Packages release notes 见
...
- `foundation:doctor` 离线诊断 CLI。
- `pnpm create vinext-app` 脚手架（位于 `tools/create-vinext-app/`）。
...
- 文档中标注的 owner 占位（`OWNER`）将在仓库绑定 GitHub origin 后替换。
