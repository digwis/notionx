...
> 本文档是 `@vinext/foundation` 的架构概览。详细的设计动机与权衡分析见
> [`docs/superpowers/specs/2026-06-10-foundation-package-design.md`](../../superpowers/specs/2026-06-10-foundation-package-design.md)；
> 分阶段迁移历史见同名实现计划。如果只是想用，最短路径是
> [创建新项目](./creating-new-project.md)。

## 简介

`@vinext/foundation` 是 vinext 项目的可复用平台层。它把"所有 vinext 项目都需要
...
仓库根是一个 pnpm workspace，把"包"、"应用"、"脚手架工具"分别放到三个顶层目录：

```text
vinext-monorepo/
├── packages/
│   └── foundation/        # 编译并发布为 @vinext/foundation
│       ├── src/
...
├── apps/
│   └── starter/           # 业务应用：定义内容源、公开路由、领域 Admin 页
│       ├── app/           # blog/、movies/、领域 Admin 子页、page.tsx
│       ├── lib/
│       │   ├── content/   # defineContentSource(blog, ...) + defineContentSource(movies, ...)
│       │   ├── admin/nav.ts          # createAdminNav([...])
│       │   ├── auth.config.ts        # AuthConfig
│       │   └── site/config.ts
│       ├── components/    # shadcn/ui primitives + 业务组件
│       ├── migrations/    # D1 schema（auth + 业务）
│       ├── public/
│       ├── wrangler.jsonc
│       ├── vite.config.ts
│       ├── next.config.ts
│       └── package.json   # "@vinext/foundation": "workspace:*"
├── tools/
│   └── create-vinext-app/ # `pnpm create vinext-app` 脚手架（私有 workspace）
├── pnpm-workspace.yaml
├── package.json           # 根脚本：pnpm -r build、pnpm -r test
├── .changeset/            # 版本管理
└── .github/workflows/
    ├── release.yml        # 推 main 后发布到 GitHub Packages
...
| 7 | `worker` | 入口：`createFoundationWorker` | admin 及以下 |
...
项目通过四个 TypeScript 接口消费本包。每个接口都在 `@vinext/foundation/types`
...
| `WorkerOptions` | `src/types.ts` | 传给 `createFoundationWorker()` 的总配置对象，聚合 `ContentSource[]`、`adminNav`、`authConfig`、`siteConfig`、`extraRoutes`。 |
...
`createFoundationWorker(options)` 装配。

## 分发模型

包发布到 **GitHub Packages**，scope 为 `@vinext`。

- **版本管理**：changesets。任何影响 `packages/foundation/**` 的 PR 都必须携带
  一个 changeset 文件，描述变更内容与 semver bump 类型。
- **发布流程** (`.github/workflows/release.yml`)：推 main 时，如果
  `packages/foundation/**` 有变化，执行 `pnpm changeset version` →
...
- **消费方升级**：每个消费项目（脚手架新建的独立项目）的
  `.github/dependabot.yml` 会在 `@vinext/foundation` 出现 minor/patch 升级时打开
  PR。详见 [升级 Foundation](./upgrading-foundation.md)。
- **私有预览**：canary 通过 `pnpm changeset publish --tag=next` 走 GitHub
  Packages；不需要自建 Verdaccio。

`pnpm-workspace.yaml` 声明三个 workspace 目录（`packages/*`、`apps/*`、
`tools/*`），并通过 `link-workspace-packages=true` 在 monorepo 内零成本引用。
发布前会以 `workspace:*` 形式链接；发布后通过真实的版本号引用。

## 测试与 CI 保障

- **单元测试**：包用 vitest（`packages/foundation/vitest.config.ts`），覆盖
...
- **Doctor**：`pnpm --filter @vinext/foundation foundation:doctor` 离线诊断
...
进入包的内容集中在 `packages/foundation/src/` 下；包内子模块的归属可参考
[依赖分层](#七层依赖分层)那张表。
...
- 想要升级 `@vinext/foundation`：[升级 Foundation](./upgrading-foundation.md)
- 想要查阅历史变更：[Foundation Changelog](./foundation-changelog.md)
