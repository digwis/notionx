...
> - **不破坏**已有数据
>
> 全部在 `nextion add` / `nextion remove` / `nextion update` 三个命令下闭环。
...
| **Astro integrations** | `astro add` / `astro remove` / `astro upgrade` 三个命令分工；integration 自身可以被更新；不强制依赖、不破坏老项目 | 我们的 `nextion add` / `nextion remove` / `nextion update` 一一对应 |
| **Prisma migrate** | `schema.prisma` 是**单点真理**；`prisma migrate dev` 生成不可变 migration 文件；`prisma db pull` 做**反向同步**（从真实数据库推导出 schema） | `.nextion/registry.json` 是**安装侧**单点真理；`.nextion/migrations/*.sql` 是**运行时侧**不可变变更；`nextion pull` 做反向同步 |
| **Angular schematics** | `collection.json` 协议；schematic 自身可以**升级**（`ng update`）；每个 schematic 是一个有版本号的转换；规则列表（Rule）让 transformations 可观察 | 我们的 `RegistryItem` 跟 `schematic` 同构；`nextion update` 用规则链（Rule chain）做 transformations |
...
```ts
// 由 create-nextion-app 写入，跨项目可移植
type RegistryItem = {
  // 身份
...
### 3.1 `.nextion/registry.json` —— 单点真理
...
```json
{
  ...
  "registries": {
    ...
    "user.local": { "url": "file:./.nextion/local-registry.json" }
  },
  "installed": [
    {
...
### 3.2 `.nextion/migrations/` —— 不可变变更历史
...
`nextion update` / `nextion add` 在改 schema 前**先 dry-run 生成 migration**，确认后才写盘 + 提示用户去 Cloudflare 跑 `wrangler d1 migrations apply`。

### 3.3 `.nextion/state/` —— 运行时状态
...
```text
       ...
       │              │                    │              │
       ▼              ▼                    ▼              ▼
   nextion add   nextion remove       nextion update   nextion pull
   (新增能力)     (删除能力)            (升级能力)       (反向同步)
```

### 4.1 `nextion add <item-id>`
...
```bash
$ nextion update           # 升级所有（包括 @notionx/core）
$ nextion update blog      # 只升级 blog
$ nextion update --dry-run # 不写盘，只输出 diff
$ nextion update --core    # 只升级 @notionx/core 运行时
```

### 4.4 `nextion pull`（可选，v2.1）
...
这是 Prisma `db pull` 的 Nextion 版本。
...
1. `nextion update` **先 dry-run**，把将要做的事按顺序打印出来
2. 用户确认（或 `--yes`）
3. 生成 `.nextion/migrations/NNNN_xxx.sql` + 更新 `_meta.json`
4. **不自动连 Notion / Cloudflare 跑 migration**（沙箱规则 + 安全）；只打印命令让用户跑
5. `wrangler d1 migrations apply` 成功后，nextion 把 migration 标记为已应用
...
- `file:./.nextion/local-registry.json`（本地）
- `git:github.com/team/nextion-private-registry`（git）

第三方团队可以发布**自己的内容模型市场**，跟 Notion 模板市场一样。

### 5.5 Codemod 是 versioned 的

每个 `ts-code-transform` step 引用一个**带版本的 codemod 名**。nextion 内置：
...
| `nextion update` 已实装 | 沿用 + 加强（codemod + migration） |
| `nextion add` 只有测试和计划 | 完整实现（registry 拉取 + 模板渲染 + migration 生成） |
| 没有 `nextion remove` | 完整实现（保留数据 + 可选 purge） |
| 没有 `nextion pull` | 可选 v2.1 |
| 没有 migration 文件系统 | 引入 `.nextion/migrations/`（Prisma 风格） |
...
### PR 3: `nextion add` / `nextion remove`
- 完整实现 add 命令（注册到 CLI）
- 完整实现 remove 命令
- 实现内置的 `blog` / `docs` / `search` / `movies` 四个 RegistryItem
- 跑迁移测试

### PR 4: `nextion update` 强化（migration + codemod）
- 引入 `.nextion/migrations/` 目录
- 引入 codemod runner（`@nextion/codemods` 包）
- 强化 update 命令：先 dry-run → 生成 migration → 应用 codemod → 渲染新文件
- 跟 `@notionx/core` 1.x → 2.x 升级一起测

## 8. 命名与定位

- 协议名：**Nextion Registry Protocol v2**
- 配置文件：`.nextion/registry.json`
- 命令：`nextion add` / `nextion remove` / `nextion update` / `nextion diff` / `nextion doctor`
- 包：`@notionx/core`（运行时）+ `@notionx/create-nextion-app`（脚手架 + CLI）
- 远程 registry：`https://registry.nextion.dev/`（未来）
