...
| `@notionx/create-nextion-app` | 脚手架和 `nextion` 维护 CLI | 查看 `packages/create-nextion-app/package.json` |
| `create-nextion-app` | `npm create nextion-app` 入口 shim | 查看 `packages/create-nextion-app-shim/package.json` |
| `@notionx/skill` | nextion skill 安装器 | 查看 `packages/nextion-skill/package.json` |
...
```text
  ✓  npm publish WILL run for this push.
    Packages to bump: @notionx/create-nextion-app (0.7.0 → ?)
    ...
```
或
```text
...
npm view create-nextion-app version
