// tools/create-vinext-app/src/prompt.ts
//
// Interactive prompt for `create-vinext-app`. Uses `@clack/prompts` to
...
export interface Answers {
  ...
  /**
   * Dependency specifier used for `@vinext/foundation` in the
   * generated `package.json`. Default: `"workspace:*"`. The CLI
   * `--foundation-source` flag overrides this for non-monorepo
   * smoke tests.
   */
...
export async function prompt(argv: string[] = process.argv): Promise<Answers> {
  p.intro("create-vinext-app — scaffold a new vinext project");
