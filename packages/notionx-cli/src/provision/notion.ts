// packages/notionx-cli/src/provision/notion.ts
//
...
/**
 * Convert relation property definitions from the flat "create" format to the
 * format required by PATCH /v1/data_sources/{id}.
 *
 * Notion's PATCH /v1/data_sources endpoint requires relation properties to
 * carry both the flat `data_source_id` (for the database reference) and the
 * explicit `type` + `single_property` wrapper (for the relation kind).
 * Omitting either side triggers a validation_error.
 */
function toDataSourcePatchProperties(properties: NotionPropertyMap): NotionPropertyMap {
  const patched: NotionPropertyMap = {};
  for (const [name, definition] of Object.entries(properties)) {
    if (
      isRecord(definition) &&
      notionPropertyDefinitionType(definition) === "relation" &&
      isRecord(definition.relation) &&
      typeof definition.relation.data_source_id === "string"
    ) {
      patched[name] = {
        relation: {
...
async function findExistingDatabaseByStableKey(input: {
  ...
  const matches = (raw.results ?? [])
    .filter((item): item is Record<string, unknown> => {
      ...
      if (item.in_trash === true) return false;
...
async function ensureDataSourceProperties(input: {
  ...
  await runOrThrowNtn(
    [
      ...
      "PATCH",
      "-d",
      JSON.stringify({ properties: toDataSourcePatchProperties(missing.properties) }),
    ],
    { env: { NOTION_API_TOKEN: input.apiToken } }
  );
  ...
  if (Object.keys(nonTitleProps).length > 0) {
    await runOrThrowNtn(
      [
        ...
        "PATCH",
        "-d",
        JSON.stringify({ properties: toDataSourcePatchProperties(nonTitleProps) }),
      ],
      { env: { NOTION_API_TOKEN: input.apiToken } }
    );
  }
...
export interface NotionPageRef {
  id: string;
  url: string;
  /** True when this run created the page; false when an existing one was reused. */
  created: boolean;
}

/**
 * Find or create a Notion page with the given title under the given parent.
 *
 * We first search existing pages by title and reuse one whose direct parent
 * matches `parentPageId`. This keeps re-runs of the scaffolder from leaving
 * behind duplicate category pages ("Content", "Blog", etc.).
 */
export async function ensureNotionPage(input: {
  apiToken: string;
  parentPageId: string;
  title: string;
  ...
  const raw = JSON.parse(stdout) as { results?: unknown[] };
  const expectedParent = compactNotionId(input.parentPageId);
  const match = (raw.results ?? []).find((item): item is Record<string, unknown> => {
    if (!isRecord(item)) return false;
    if (item.object !== "page") return false;
    const parent = item.parent;
    if (!isRecord(parent)) return false;
    const pageId = typeof parent.page_id === "string" ? parent.page_id : null;
    return pageId !== null && compactNotionId(pageId) === expectedParent;
  });
  ...
  const createStdout = await runOrThrowNtn(
    ["api", "v1/pages", "-d", JSON.stringify(body)],
    { env: { NOTION_API_TOKEN: input.apiToken } }
  );
