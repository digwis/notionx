// packages/notionx-cli/src/provision/index.ts
//
...
export async function provision(
  ...
  try {
    ...
    if (resolvedToken) {
      ...
      if (notionInputs) {
        const { content, pages, blocks, categoryPageIds } =
          await provisionNotionContentAndPages({
            answers,
        ...
        // Created under its own category page at the top level.
        // Disable with `--no-site-settings`.
        let siteSettingsDataSourceId: string | undefined;
        let siteSettingsCategoryPageId: string | undefined;
        const needsSiteSettingsPage =
          answers.enableSiteSettings || shouldCreateTranslationSources(answers);
        if (needsSiteSettingsPage) {
          const siteSettingsPage = await ensureNotionPage({
            apiToken: notionInputs.apiToken,
            parentPageId: notionInputs.parentPageId,
            title: "Site Settings",
          });
          siteSettingsCategoryPageId = siteSettingsPage.id;
        }
        if (answers.enableSiteSettings) {
          const settings = await ensureSiteSettingsDatabase({
            apiToken: notionInputs.apiToken,
            parentPageId: siteSettingsCategoryPageId!,
        ...
        // enabled. Each source is created under the same category
        // page as its base data source so translations stay grouped
        // with the content they translate.
        if (shouldCreateTranslationSources(answers)) {
          const translationRefs = await provisionTranslationSources({
            apiToken: notionInputs.apiToken,
            parentPageIds: {
              "blog-translations": categoryPageIds.content,
              "page-translations":
                categoryPageIds.pages ?? notionInputs.parentPageId,
              "block-translations":
                categoryPageIds.blocks ?? notionInputs.parentPageId,
              "site-settings-translations":
                siteSettingsCategoryPageId ?? notionInputs.parentPageId,
            },
            baseDataSourceIds: {
      ...
      if (notion) {
        ...
        // Site Settings category page is only needed here when
        // bilingual mode is enabled (translations live under it).
        let siteSettingsCategoryPageId: string | undefined;
        if (shouldCreateTranslationSources(answers)) {
          const siteSettingsPage = await ensureNotionPage({
            apiToken: notion.apiToken,
        ...
        if (shouldCreateTranslationSources(answers)) {
          const translationRefs = await provisionTranslationSources({
            ...
            parentPageIds: {
              "blog-translations": categoryPageIds.content,
              "page-translations":
                categoryPageIds.pages ?? notion.parentPageId,
              "block-translations":
                categoryPageIds.blocks ?? notion.parentPageId,
              "site-settings-translations":
                siteSettingsCategoryPageId ?? notion.parentPageId,
            },
            baseDataSourceIds: {
              "blog-translations": content.dataSourceId,
              "page-translations":
...
async function provisionNotionContentAndPages({
  ...
  // Create category pages directly under the user-selected parent page
  // so databases don't all end up as flat siblings. Each category page
  // holds its base database plus its translations.
  //
  //   Parent Page
  //     ├── Blog
  ...
  //     ├── Blocks
  //     │     └── Blocks + Block Translations
  //     └── Site Settings
  //           └── Site Settings + Site Settings Translations
  const bilingual = shouldCreateTranslationSources(answers);

  const contentCategoryPage = await ensureNotionPage({
    apiToken,
    parentPageId,
    title: answers.contentSource.title,
  });

  const blocksCategoryPage = answers.enableBlocks || bilingual
    ? await ensureNotionPage({
        apiToken,
        parentPageId,
        title: "Blocks",
      })
  ...
  const pagesCategoryPage = answers.enablePages || bilingual
    ? await ensureNotionPage({
        apiToken,
        parentPageId,
        title: "Pages",
  ...
  const content = await ensureNotionDatabase({
    apiToken,
    parentPageId: contentCategoryPage.id,
    title: answers.contentSource.title,
  ...
  const blocks = answers.enableBlocks
    ? await ensureBlocksDatabase({
        apiToken,
        parentPageId: blocksCategoryPage!.id,
        projectName: answers.projectName,
        contentSourceId: answers.contentSource.id,
        contentSourceTitle: answers.contentSource.title,
        contentSourceListPath: `/${slugify(answers.contentSource.title) || answers.contentSource.id}`,
        locale: answers.defaultLocale,
      })
    : {
        ok: true,
  ...
  const pages = answers.enablePages
    ? await ensurePagesDatabase({
        apiToken,
        parentPageId: pagesCategoryPage!.id,
        projectName: answers.projectName,
        contentSourceId: answers.contentSource.id,
        contentSourceTitle: answers.contentSource.title,
        contentSourceListPath: `/${slugify(answers.contentSource.title) || answers.contentSource.id}`,
  ...
  return {
    ...
    blocks,
    categoryPageIds: {
      content: contentCategoryPage.id,
      pages: pagesCategoryPage?.id,
      blocks: blocksCategoryPage?.id,
    },
  };
}
  ...
  for (const { modelId, envVar } of translationModels) {
    try {
      const trans = await ensureTranslationDatabase({
        ...
        parentPageId: input.parentPageIds[modelId],
        modelId,
