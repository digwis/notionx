...
describe("sample page builders", () => {
  ...
  it("builds Pages database properties and a navigation-aware home page", () => {
    ...
    expect(properties.Blocks).toEqual({ relation: {} });
        ...
        results: [
          {
            object: "data_source",
            id: "ds-stable",
            title: [{ plain_text: "Site Settings" }],
...
describe("ensureNotionPage", () => {
  beforeEach(() => {
    runOrThrowNtnMock.mockReset();
  });

  it("reuses an existing page with matching title and parent", async () => {
    runOrThrowNtnMock.mockResolvedValueOnce(
      JSON.stringify({
        results: [
          {
            object: "page",
            id: "page-content-1234",
            parent: { type: "page_id", page_id: "page-parent-1234" },
            url: "https://www.notion.so/page-content-1234",
          },
        ],
      })
    );

    const result = await ensureNotionPage({
      apiToken: "token",
      parentPageId: "page-parent-1234",
      title: "Content",
    });

    expect(result.created).toBe(false);
    expect(result.id).toBe("page-content-1234");
    expect(result.url).toBe("https://www.notion.so/page-content-1234");
    expect(runOrThrowNtnMock).toHaveBeenCalledTimes(1);
    expect(runOrThrowNtnMock).not.toHaveBeenCalledWith(
      expect.arrayContaining(["v1/pages"]),
      expect.anything()
    );
  });

  it("creates a new page when no matching page exists", async () => {
    runOrThrowNtnMock
      .mockResolvedValueOnce(JSON.stringify({ results: [] }))
      .mockResolvedValueOnce(
        JSON.stringify({
          id: "page-new-1234",
          url: "https://www.notion.so/page-new-1234",
        })
      );
    ...
    expect(result.id).toBe("page-new-1234");

    const createCall = runOrThrowNtnMock.mock.calls.find(
      (call) =>
        Array.isArray(call[0]) && (call[0] as string[]).includes("v1/pages")
    );
    expect(createCall).toBeDefined();
    const body = JSON.parse((createCall![0] as string[])[3]);
    expect(body.parent).toEqual({
      type: "page_id",
      page_id: "page-parent-1234",
    });
    expect(body.properties.title.title[0].text.content).toBe("Content");
  });

  it("ignores pages whose parent does not match", async () => {
    runOrThrowNtnMock
      .mockResolvedValueOnce(
        JSON.stringify({
          results: [
            {
              object: "page",
              id: "page-wrong-parent",
              parent: { type: "page_id", page_id: "page-other-1234" },
            },
          ],
        })
      )
      .mockResolvedValueOnce(
        JSON.stringify({ id: "page-new-1234", url: "https://www.notion.so/page-new-1234" })
      );
