// packages/notionx-cli/src/registry/uninstall.test.ts
...
describe("uninstallItem", () => {
  ...
  it("feature-module: dry-run reports fallback files without writing", async () => {
    ...
    const modelsBefore = "// existing models sentinel\n";
    await writeFile(modelsPath, modelsBefore, "utf8");
    const blocksPath = path.join(dir, "components/page-blocks.tsx");
    await mkdir(path.dirname(blocksPath), { recursive: true });
    const blocksBefore = "// existing blocks sentinel\n";
    await writeFile(blocksPath, blocksBefore, "utf8");
    ...
    expect(await fs.readFile(modelsPath, "utf8")).toBe(modelsBefore);
    expect(await fs.readFile(blocksPath, "utf8")).toBe(blocksBefore);
  });
});
