// packages/notionx-cli/src/registry/doctor.ts
//
...
export async function runProjectDoctor(input: RunDoctorInput): Promise<DoctorReport> {
  ...
  if (!existsSync(registryPath)) {
    checks.push({
      ...
      hint: "This directory is not a v2 notionx project. Run `npm create notionx@latest` to scaffold one.",
    });
    ...
    try {
      ...
      if (coreDep && coreDep !== manifest.notionxCore) {
        checks.push({
          ...
          hint: "These should usually match; run `notionx update --core` to align package.json and registry metadata.",
        });
      } else {
        checks.push({
          id: "core.aligned",
