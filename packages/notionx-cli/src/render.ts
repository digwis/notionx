// packages/notionx-cli/src/render.ts
//
// File renderer for `@notionx/cli`. Walks a selected starter
// template directory (for example `src/templates/blog`), copies static
// files verbatim, and renders `.tmpl` files with simple `{{token}}`
// interpolation. The token map is derived from the prompt answers.
...
async function buildTokenMap(
  ...
  return {
    ...
    notionxCliVersion:
      answers.notionxSource === MONOREPO_PROTOCOL
        ? MONOREPO_PROTOCOL
        : `^${scaffoldVersion}`,
    contentSourceId: id,
...
export async function resolveStarterTemplatesDir(
  starter: string,
): Promise<string> {
  const templatesRoot = await resolveTemplatesDir();
  const starterDir = path.join(templatesRoot, starter);
...
/**
 * Render the project. `templatesDir` is the absolute path to the
 * selected starter's templates directory inside the scaffolder
 * package; `outDir` is the target directory the user picked in the
 * prompt (relative or absolute). Existing directories are not
 * removed — the caller is responsible for picking a clean target.
 */
